...
export function YouLearnChatPage({ userId }: YouLearnChatPageProps) {
  ...
  const [showSidebar, setShowSidebar] = useState(true); // Default to true for better UX
  ...
  useEffect(() => {
    ...
    // Set up periodic refresh for session titles
    const interval = setInterval(() => {
      if (currentSession) {
        loadSessions();
      }
    }, 10000); // Refresh every 10 seconds when there's an active session
  ...
  const selectSession = (session: ChatSession) => {
    if (currentSession?.id === session.id) {
      // If clicking on the same session, just close sidebar
      setShowSidebar(false);
      return;
    }
            ...
            <div className="w-full max-w-5xl mx-auto">
              ...
              >
                {sessions.map((session: ChatSession, index: number) => (
                  ...
                  >
                    <Card
                      className="p-6 cursor-pointer hover:shadow-xl transition-all duration-300 border-2 hover:border-primary group relative overflow-hidden w-full"
  ...
  return (
    <div className="h-screen flex relative">
      ...
      <AnimatePresence>
        {showSidebar && (
          <motion.div
            ...
            className="w-80 bg-gray-50 border-r flex flex-col h-full z-50"
          >
      ...
      {/* Main chat interface */}
      <div className={cn("flex-1 relative", !showSidebar && "flex items-center justify-center")}>
        ...
        <div className={cn("h-full", !showSidebar && "max-w-6xl mx-auto px-4")}>
          <ModernChatInterface 
            ...
            onNewContent={(content: string, type: 'upload' | 'paste' | 'record') => {
              ...
              // Update session last_message_at and reload sessions to get any title updates
              supabase
                .from('chat_sessions')
                ...
                .then(() => {
                  loadSessions();
                  // Reload sessions after a short delay to catch title updates
