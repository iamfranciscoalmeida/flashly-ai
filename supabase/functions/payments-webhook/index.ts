...
async function handleSubscriptionCreated(supabaseClient: any, event: any) {
  ...
  // Update user metadata to indicate premium status
  const planInterval = subscription.items.data[0]?.plan.interval || "month";
...
async function handleSubscriptionUpdated(supabaseClient: any, event: any) {
  ...
  if (subscriptionData?.user_id) {
    ...
    const { error: userUpdateError } =
      await supabaseClient.auth.admin.updateUserById(subscriptionData.user_id, {
        user_metadata: {
          is_pro: isPro,
          plan: isPro ? planInterval : null,
          period_end: isPro ? subscription.current_period_end : null,
        },
      });

    if (userUpdateError) {
      console.error("Error updating user metadata:", userUpdateError);
      // Continue anyway as the subscription was updated successfully
    }
  }
...
async function handleSubscriptionDeleted(supabaseClient: any, event: any) {
  ...
  try {
    // Get the user ID before updating the subscription status
    const { data: subscriptionData } = await supabaseClient
      .from("subscriptions")
      .select("user_id")
      .eq("stripe_id", subscription.id)
    ...
    if (userId) {
      ...
      if (userUpdateError) {
        console.error("Error updating user metadata:", userUpdateError);
        // Continue anyway as the subscription was canceled successfully
      }
    }

    // If we have email in metadata, update user's subscription status in users table
